...
namespace Aspose.Cells.UI.Config
{
    ...
    public class AsposeToolsContext : Context
    {
        ...
        /// </summary>
        /// <param name="hc"></param>
        internal AsposeToolsContext(HttpContext hc) : base(hc)
        {
            Thread.SetData(Thread.GetNamedDataSlot(Configuration.ResourceFileSessionName + hc.Request.Url.Host.Trim().Replace(".", "") + "ContextSlot"), hc);
        }
        ...
        /// </summary>
        internal static AsposeToolsContext atcc
        {
            set => atc = value;
        }

        internal static AsposeToolsContext Current => atc;

        FlexibleResources _FlexibleResources;

        internal FlexibleResources Resources => _FlexibleResources ?? (_FlexibleResources = new FlexibleResources(_context, Locale, BaseResources));
    }
}