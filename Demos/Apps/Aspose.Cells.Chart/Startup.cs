using System.IO;
using Aspose.Cells.Chart.Controllers;
...
namespace Aspose.Cells.Chart
{
    public class Startup
    {
        private static readonly AppNames appName = AppNames.Chart;
        ...
        {
            services.AddScoped<ICellsController, ChartController>();
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILogger<Startup> logger)
        {
            ...
            app.UseEndpoints(endpoints =>
            {
                if (Configuration.IsAsposeCloudApp)
                {
                    endpoints.MapControllerRoute("ChartCloudApi", "cells/" + AppName + "/api/ChartApi/{action}",
                        new { controller = "ChartCloudApi", action = AppName });
                }
                else {
                    endpoints.MapControllerRoute("ChartApi", "cells/" + AppName + "/api/ChartApi/{action}",
                       new { controller = "ChartApi", action = AppName });
                }
            });
            app.UseEndpoints(endpoints =>
            {
