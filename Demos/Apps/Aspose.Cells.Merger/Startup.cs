...
using Aspose.Cells.Merger.Controllers;
...
namespace Aspose.Cells.Merger
{
    public class Startup
    {
        private static readonly AppNames appName = AppNames.Merger;
        ...
        {
            services.AddScoped<ICellsController, MergerController>();
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILogger<Startup> logger)
        {
            ...
            app.UseEndpoints(endpoints =>
            {
                if (Configuration.IsAsposeCloudApp)
                {
                    endpoints.MapControllerRoute("MergerCloudApi", "cells/" + AppName + "/api/MergerApi/{action}",
                        new { controller = "MergerCloudApi", action = AppName });
                }
                else
                {
                    endpoints.MapControllerRoute("MergerApi", "cells/" + AppName + "/api/MergerApi/{action}",
                        new { controller = "MergerApi", action = AppName });
                }
            });
            app.UseEndpoints(endpoints =>
            {
            ...
            app.UseEndpoints(endpoints =>
            {
                endpoints.MapControllerRoute("Delete", "cells/" + AppName + "/api/Common/Delete",
                    new {controller = "Common", action = "Delete"});
            });
        }
    }
}