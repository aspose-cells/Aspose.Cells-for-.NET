...
{
    public class AssemblyCloudApiController : CellsCloudApiControllerBase
    {
        private const string AppName = "Assembly";
        public AssemblyCloudApiController(IStorageService storage, IConfiguration configuration, ILogger<AssemblyCloudApiController> logger) : base(storage, configuration, logger)
        {
        }
        ...
        public async Task<Response> Assemble(string datasourceName)
        {
            ...
            return await RunBusinessCode(async (IDictionary<string, Stream> docs, string sessionId,string datasourceName) =>
            {
                ...
                try
                {
                    ...
                    var value =cells.Assembly(docs, datasourceName);
