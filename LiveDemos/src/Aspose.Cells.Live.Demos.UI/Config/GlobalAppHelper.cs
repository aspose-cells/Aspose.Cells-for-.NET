...
namespace Aspose.Cells.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeCellsContext.atcc = new AsposeCellsContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeCellsContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeCellsContext.Current.Cache.Add(
				   SessionID,
					resources,
